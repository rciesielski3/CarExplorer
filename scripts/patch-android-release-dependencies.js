...
if (fs.existsSync(packagePath)) {
  ...
  if (!androidSdkVersions) {
    console.warn(
      "Could not find sdkVersions.android in react-native-google-mobile-ads package.json; skipping SDK patch.",
    );
  } else if (androidSdkVersions.googleMobileAds === targetVersion) {
...
if (fs.existsSync(bundleHermesTaskPath)) {
  ...
  const taskPatches = [
    {
      ...
      replacement:
        "          // Keep Metro cache for reliable local release builds.",
      label: "Metro cache reset",
    },
    {
      line: '          add("--verbose")',
      replacement:
        "          // Keep Expo export non-verbose to avoid local Metro stalls.",
  ...
  for (const patch of taskPatches) {
    if (updatedTaskSource.includes(patch.line)) {
      updatedTaskSource = updatedTaskSource.replace(
        patch.line,
        patch.replacement,
      );
      console.log(
        `Disabled ${patch.label} in React Native Gradle bundle task.`,
      );
    } else {
      console.log(
