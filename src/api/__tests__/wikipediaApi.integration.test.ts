...
describe("Wikipedia API with Wikidata Fallback Integration", () => {
  ...
  it("falls back to Wikidata when Wikipedia fails for a car", async () => {
    ...
    mockFetch
      // Wikidata search (called FIRST now)
      ...
      // Wikidata specs fetch (called in parallel, happens immediately after search)
      ...
      .mockResolvedValueOnce(errorScenarios.notFound404({}))
      // Wikidata description fetch (called when Wikipedia fails)
      .mockResolvedValueOnce(
    ...
    // 1 wikidata search + 1 wikidata specs (parallel) + 3 candidates × 2 calls + 1 search + 3 make candidates × 2 + 1 description = 16 calls
  ...
  it("uses Wikipedia result when available (Wikidata not called)", async () => {
    ...
    mockFetch
      ...
      // Wikipedia details extract succeeds on first candidate
      .mockResolvedValueOnce(
        wikipediaDetailsResponse({
    ...
    // 1 wikidata search + 1 Wikipedia details extract call (first candidate succeeds)
  ...
  it("caches Wikidata results to avoid repeated API calls", async () => {
    mockFetch
      // First request: Wikidata search succeeds (called FIRST now)
      .mockResolvedValueOnce(
        wikidataSearchResponse([
          {
            id: "Q999999",
    ...
    // First call makes 16 API calls (1 wikidata search + 1 wikidata specs (parallel) + 3 candidates × 2 + search + 3 make × 2 + 1 description), second uses cache (0 more calls)
    expect(mockFetch).toHaveBeenCalledTimes(16);
  });
});
